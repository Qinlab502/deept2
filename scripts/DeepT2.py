...
#     test_filter_y_[test_filter_y_ == 'unknown'] = -1
    ...
    # Get output file
    with open(output_path + '/' + prefix + '_prediction.txt', 'w') as f1:
        if -1 not in label['isolation_predict'].values:
            ...
            f1.write('Found ' + str(len(test_out)) + ' out-of-distribution ksb sequence(s)' + '\n')
            for i in range(len(test_out)):
                f1.write(headers[index[test_out[i]]] + '\n')
                

        test_in = label[label['isolation_predict'] == 1].index.tolist()
        ys_pred = None  # Initialize ys_pred with a default value
        if len(test_in) >0:
            with torch.no_grad():
            ...
            f1.write('Found ' + str(len(test_in)) + ' in-distribution ksb sequence(s)' + '\n')
            ...
            f1.write('No in_distribution ksb sequence(s) found.' + '\n')
